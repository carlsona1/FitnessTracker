...
/* async function getMET(id){
    const sql = `SELECT Exercise_Type FROM ${PREFIX}Comments WHERE id=?`;
    const rows = await mysql.query(sql, [id]);
...
async function add(Owner_id, Privacy_Setting, Start_Time, End_Time, Exercise_Type, Note, Distance, Sets, Reps_Per_Set, Weight, calories){
   ...
   //const calories = (End_Time - Start_Time) * MET * 3.5 * userAtr.weight / (200*60) ;
    const sql = `INSERT INTO ${PREFIX}Comments (created_at, Owner_id, Privacy_Setting, Start_Time, End_Time, Exercise_Type, Note, Distance, Sets, Reps_Per_Set, Weight, calories) VALUES ? ;`;
...
const search = async q => await mysql.query(`SELECT * FROM ${PREFIX}Comments WHERE MONTH(End_Time) AND DAY(End_Time) LIKE ? OR id LIKE ? OR Exercise_Type LIKE ?; `, [`%${q}%`, `%${q}%`, , `%${q}%`]);
